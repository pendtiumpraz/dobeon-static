...
export default function Card({
  ...
  const getGradientClass = () => {
    if (gradient === 'blue') {
      return 'bg-gradient-to-br from-primary via-primary-dark to-primary text-white shadow-xl shadow-primary/20 border-none';
    }
    if (gradient === true) {
      return 'bg-gradient-to-br from-primary to-primary-dark text-white shadow-xl shadow-primary/20 border-none';
    }
    return 'glass-card hover:bg-white/60 border border-white/50 hover:border-primary/20';
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative p-6 md:p-8">
        {icon && (
          <div className={`mb-5 inline-flex items-center justify-center w-14 h-14 rounded-xl transition-all duration-300 ${hover ? 'group-hover:scale-110 group-hover:rotate-3' : ''
            } ${isGradient
        ...
        {hover && (
          <div className={`mt-6 flex items-center gap-2 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-0 group-hover:translate-x-2 ${isGradient ? 'text-white' : 'text-primary'
            }`}>
